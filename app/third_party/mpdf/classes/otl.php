            ...
            if (($useOTL & 0xFF) && $GSUBscriptTag && $GSUBlangsys && $GSUBFeatures) {
                ...
                if ($this->shaper == 'A') {
                    ...
                    for ($i = 0; $i < count($this->OTLdata); $i++) {
                        ...
                        else if ($this->OTLdata[$i]['uni'] == 0xFEAE || $this->OTLdata[$i]['uni'] == 0xFEF2 || $this->OTLdata[$i]['uni'] == 0xFEF0 || $this->OTLdata[$i]['uni'] == 0xFEF4 || $this->OTLdata[$i]['uni'] == 0xFBE9
            ...
            while ($ptr < (count($this->OTLdata))) { // whilst there is another glyph ..0064
                ...
                foreach ($this->GSUBLookups[$lu]['Subtables'] AS $c => $subtable_offset) {
                    ...
                    if (isset($this->GSLuCoverage[$lu][$c][$currGID])) {
                        ...
                        $shift = $this->_applyGSUBsubtable($lu, $c, $ptr, $currGlyph, $currGID, ($subtable_offset - $this->GSUB_offset), $Type, $Flag, $MarkFilteringSet, $this->GSLuCoverage[$lu][$c],
                                                           0, $tag, 0, $tagInt);
            ...
            while ($ptr < (count($this->OTLdata))) { // whilst there is another glyph ..0064
                ...
                foreach ($LookupList AS $lu => $tag) {
                    ...
                    foreach ($this->GSUBLookups[$lu]['Subtables'] AS $c => $subtable_offset) {
                        ...
                        if (isset($this->GSLuCoverage[$lu][$c][$currGID])) {
                            // Get rules from font GSUB subtable
                            $shift = $this->_applyGSUBsubtable($lu, $c, $ptr, $currGlyph, $currGID, ($subtable_offset - $this->GSUB_offset), $Type, $Flag, $MarkFilteringSet,
                        ...
                        // First check if current glyph is a Halant/Virama
                            ...
                            if (strpos('pref blwf pstf', $usetag) !== false) {
                                ...
                                if (isset($this->GSLuCoverage[$lu][$c][$nextGID])) {
                                    ...
                                    // Get rules from font GSUB subtable
                                    $shift = $this->_applyGSUBsubtableSpecial($lu, $c, $ptr, $currGlyph, $currGID, $nextGlyph, $nextGID, ($subtable_offset - $this->GSUB_offset), $Type,
                        ...
                        if ($matched) {
                            ...
                            for ($p = 0; $p < $SubstCount; $p++) {
                                ...
                                foreach ($this->GSUBLookups[$lu]['Subtables'] AS $luc => $lusubtable_offset) {
                                    $shift = $this->_applyGSUBsubtable($lu, $luc, $luptr, $lucurrGlyph, $lucurrGID, ($lusubtable_offset - $this->GSUB_offset), $luType, $luFlag, $luMarkFilteringSet,
                        ...
                        for ($b = 0; $b < $SubClassRuleCnt; $b++) {  // EACH RULE
                            ...
                            if ($matched) {
                                ...
                                for ($p = 0; $p < $SubstCount; $p++) {
                                    ...
                                    foreach ($this->GSUBLookups[$lu]['Subtables'] AS $luc => $lusubtable_offset) {
                                        $shift = $this->_applyGSUBsubtable($lu, $luc, $luptr, $lucurrGlyph, $lucurrGID, ($lusubtable_offset - $this->GSUB_offset), $luType, $luFlag,
                                                                           $luMarkFilteringSet, $this->GSLuCoverage[$lu][$luc], 1, $currentTag, $is_old_spec, $tagInt);
                                        if ($shift) {
                                            break;
                                        }
                                    }
...
////////////////////////////////////////////////////////////////
    ...
    function get_arab_glyphs($char, $type, &$chars, $i, $scriptTag, $usetags) {
        ...
        if (($type === 0 && strpos($usetags, 'isol') === false) || ($type === 1 && strpos($usetags, 'fina') === false) || ($type === 2 && strpos($usetags, 'init') === false) || ($type === 3 && strpos($usetags,
                                                                                                                                                                                                        'medi') === false)) {
            return array($char, 0);
        }
                ...
                foreach ($this->GPOSLookups[$lu]['Subtables'] AS $c => $subtable_offset) {
                    ...
                    if (isset($this->LuCoverage[$lu][$c][$currGID])) {
                        ...
                        if (isset($this->OTLdata[$ptr]['bidi_type'])) {  // No need to check bidi_type - just a check that it exists
                            $shift = $this->_applyGPOSsubtable($lu, $c, $ptr, $currGlyph, $currGID, ($subtable_offset - $this->GPOS_offset + $this->GSUB_length), $Type, $Flag, $MarkFilteringSet,
                                                               $this->LuCoverage[$lu][$c], $tag, 0, $is_old_spec);
                            if ($shift) {
                                break;
                            }
                        }
        ...
        ////////////////////////////////////////////////////////////////////////////////
            ...
            if ($matchedpos !== false) {
                ...
                if ($offset != 0) {
                    ...
                    $LigatureWidth                                = $this->mpdf->_getCharWidth($this->mpdf->CurrentFont['cw'], $this->OTLdata[$matchedpos]['uni']) * $this->mpdf->CurrentFont['unitsPerEm']
                            / 1000; // convert back to font design units
                    $this->OTLdata[$ptr]['GPOSinfo']['BaseWidth'] = $LigatureWidth;
                    // And any intervening (ignored)characters
                    if (($ptr - $matchedpos) > 1) {
                        for ($i = $matchedpos + 1; $i < $ptr; $i++) {
                            $LigatureWidthExtra                           = $this->mpdf->_getCharWidth($this->mpdf->CurrentFont['cw'], $this->OTLdata[$i]['uni']) * $this->mpdf->CurrentFont['unitsPerEm']
                                    / 1000; // convert back to font design units
                            ...
                            if ($matched) {
                                ...
                                for ($p = 0; $p < $PosCount; $p++) {
                                    ...
                                    foreach ($this->GPOSLookups[$lu]['Subtables'] AS $luc => $lusubtable_offset) {
                                        $shift = $this->_applyGPOSsubtable($lu, $luc, $luptr, $lucurrGlyph, $lucurrGID, ($lusubtable_offset - $this->GPOS_offset + $this->GSUB_length), $luType,
                                                                           $luFlag, $luMarkFilteringSet, $this->LuCoverage[$lu][$luc], $tag, 1, $is_old_spec);
                                        if ($this->debugOTL && $shift) {
                                            $this->_dumpproc('GPOS', $lookupID, $subtable, $Type, $PosFormat, $ptr, $currGlyph, $level);
                                        }
                                        if ($shift) {
            ...
            //===========
                ...
                if ($matched) {
                    ...
                    for ($p = 0; $p < $PosCount; $p++) {
                        ...
                        foreach ($this->GPOSLookups[$lu]['Subtables'] AS $luc => $lusubtable_offset) {
                            $shift = $this->_applyGPOSsubtable($lu, $luc, $luptr, $lucurrGlyph, $lucurrGID, ($lusubtable_offset - $this->GPOS_offset + $this->GSUB_length), $luType, $luFlag,
